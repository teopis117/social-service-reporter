...
def show_input_form():
    ...
    demo_daily_logs = []
    current_date_iter = start_date_default
    days_processed_for_demo = 0; max_demo_log_entries = 15
    while current_date_iter <= end_date_default and days_processed_for_demo < max_demo_log_entries:
        if current_date_iter.weekday() < 5: 
             if random.random() < 0.9: 
                 ...
                 duration_h = random.choice([4, 4, 5, 6, 7, 8]) 
                 start_time_obj = datetime.time(start_h, start_m)
                 start_datetime_obj = datetime.datetime.combine(current_date_iter, start_time_obj)
                 end_datetime_obj = start_datetime_obj + datetime.timedelta(hours=duration_h)
                 end_time_obj = end_datetime_obj.time()
                 demo_daily_logs.append({ 'date': current_date_iter.strftime('%Y-%m-%d'), 'start_time': start_time_obj.strftime('%H:%M'), 'end_time': end_time_obj.strftime('%H:%M') })
                 days_processed_for_demo += 1
        current_date_iter += datetime.timedelta(days=1)
    if not demo_daily_logs and start_date_default <= end_date_default :
         demo_daily_logs.append({'date': start_date_default.strftime('%Y-%m-%d'), 'start_time': '09:00', 'end_time': '13:00'})
    random.shuffle(demo_daily_logs) 
    for i, log in enumerate(demo_daily_logs): log['num'] = i + 1

    activities_default = ("- Mantenimiento preventivo y correctivo a equipos de cómputo.\n"
                          "- Soporte técnico a usuarios.\n"
                          "- Actualización de software y sistemas operativos.\n"
                          "- Gestión de inventario de T.I.\n"
                          "- Documentación de procedimientos.")

    form_data_to_render = {
        **prestador_data, 
        'report_num': last_report_num + 1, 
        'period_start': period_start_str, 'period_end': period_end_str,
        'report_date_city': "Ciudad de México", 'report_date': report_date_str,
        'previous_hours': last_report_hours or 0.0,
    ...
    return render_template('input_form.html', page_title="Registro de Horas (Precargado)", data=form_data_to_render) 
...
def generate_report_pdf():
    ...
    errors = []
    required_fields = { 
        'report_num': "Número de Reporte", 'period_start': "Inicio del Periodo", 'period_end': "Fin del Periodo", 
        'report_date': "Fecha del Reporte", 'student_boleta': "Boleta", 'student_name': "Nombre Completo",
        'student_program': "Programa Académico", 'student_semester': "Semestre", 'student_email': "Correo Electrónico", 
        'prestatario_name': "Nombre del Prestatario", 'authorizing_name': "Nombre de quien Autoriza", 
        'authorizing_title': "Cargo de quien Autoriza", 'activities_description': "Descripción de Actividades", 
        'previous_hours': "Horas Acumuladas Previas"
    }
    for key, label in required_fields.items():
        if not report_data.get(key): errors.append(f"'{label}' es requerido.")
    
    report_num_int = None; previous_hours_float = 0.0; start_date=None; end_date=None; report_date_obj=None
    ...
    if not daily_logs_processed and len(log_dates) == 0: errors.append("Se requiere al menos un día de asistencia.")
    ...
    try:
        ...
        try:
             ...
             db.session.commit()
             print(f"INFO: Metadatos guardados para Reporte No. {report_num_int}")
...
# --- ESTA ES LA FUNCIÓN DE HISTORIAL COMPLETA ---
...
def report_history():
    ...
    prestador = Prestador.query.first() 
    reports = []
    boleta_usada = "N/A" # Para mostrar en la plantilla si no hay prestador
    if prestador:
        ...
        print(f"INFO (report_history): Encontrados {len(reports)} reportes para Boleta {prestador.boleta}")
         ...
         # Si no hay prestador en la BD (ej. después de un init-db sin crear el ejemplo manualmente)
         # Puedes mostrar un mensaje flash si lo deseas
         flash("No se encontró información del prestador para mostrar el historial. Genera tu primer reporte.", "info")
...
# --- ESTA ES LA FUNCIÓN DE DESCARGA COMPLETA ---
...
def download_pdf(filename):
    ...
    # Obtener la ruta absoluta a la carpeta static de la aplicación actual
    static_folder_path = Path(current_app.static_folder)
    # Construir la ruta al directorio de PDFs temporales
    temp_pdf_dir_path = static_folder_path / TEMP_PDF_FOLDER_NAME # Usa la constante global
    
    safe_dir_path = temp_pdf_dir_path.resolve() # Ruta absoluta segura
    file_path = safe_dir_path / Path(filename).name # Evitar path traversal tomando solo el nombre base
    
    print(f"INFO (download_pdf): Solicitud para descargar: {file_path}")

    # Medida de seguridad: Verificar que el archivo solicitado esté realmente dentro de la carpeta temporal
    if not str(file_path.resolve()).startswith(str(safe_dir_path)) or not file_path.is_file(): 
        print(f"ERROR (download_pdf): Acceso denegado o archivo no encontrado: {filename} en {safe_dir_path}")
    ...
    try:
        ...
        response = send_from_directory(directory=str(safe_dir_path), 
    ...
    except FileNotFoundError: # Si el archivo fue eliminado o nunca existió
        print(f"ERROR (download_pdf): Archivo no encontrado en el sistema de archivos (FileNotFoundError): {filename}")
